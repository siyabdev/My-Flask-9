...
from crud.payroll.get import get_payroll_crud, get_payrolls_crud, get_payroll_short_crud
...
#Get short details (payroll)
@payroll_get_bp.route("/short", methods=["GET"])
@require_auth
def get_payroll_short():
    data = request.json
    employee_id = data.get("employee_id")
    batch = data.get("batch")
    ...
    payroll = get_payroll_short_crud(employee_id=employee_id, batch=batch)

    try:
        if payroll:
            return PayrollShortResponse(payroll).to_dict()
